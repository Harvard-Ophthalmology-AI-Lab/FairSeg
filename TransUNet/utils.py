...
class febs_loss(nn.Module):
    def __init__(self, n_classes, n_attr=3, gamma=0.):
        super(febs_loss, self).__init__()
    ...
    def forward(self, inputs, target, attr, weight=None, softmax=False):
        ...
        for x in attr_set:
            ...
            for i in range(0, self.n_classes):
                ...
                tmp_loss += dice 
            # tmp_weights[x] = (tmp_loss/overall_loss) ** self.gamma
        ...
        # tmp_weights = torch.softmax(tmp_weights, dim=0)
        # *tmp_weights[attr][:, None, None]
        class_wise_dice = []
        loss = 0.0
        for i in range(0, self.n_classes):
...
def compute_multi_class_auc(preds: np.array, labels: np.array):
    ...
    # print(f"Multi-Class AUC: {multi_class_auc}")
    # exit()
...
def test_single_image(image, label, net, classes, multimask_output=None, patch_size=[256, 256], input_size=[224, 224],
    ...
    with torch.no_grad():
        ...
        prediction = pred
        #only for debug
        # if not os.path.exists('debug_output/images/pred'):
        #     os.makedirs('debug_output/images/pred')
        # if not os.path.exists('debug_output/images/label_disc'):
        #     os.makedirs('debug_output/images/label_disc')
        # assert prediction.shape[0] == label.shape[0]
        # mask_disc = (label>=1)
        # print(mask_disc.shape)
        # mask_disc[mask_disc==1] = 255
        # imageio.imwrite('debug_output/images/pred/pred_{}.png'.format(idx), prediction)
        # imageio.imwrite('debug_output/images/label_disc/label_{}.png'.format(idx), img_as_uint(mask_disc))
        # print('save_image')
        # exit()
        # temp = input('kkpsa')
    ...
    for i in range(1, classes + 1):
        ...
        mask_label = np.expand_dims((label== i), 0)
        mask_label = np.expand_dims(mask_label, 0)
        mask_prediction = np.expand_dims((prediction== i),0)
        mask_prediction = np.expand_dims(mask_prediction, 0)

        # hausdorff_distance = compute_hausdorff_distance(mask_label, mask_prediction)
        # hausdorff_list.append(hausdorff_distance.detach().numpy())

        attr = attr_label.detach().cpu().numpy()

        label_flat = mask_label.flatten()
        pred_flat = mask_prediction.flatten()
        attr_array = np.full(pred_flat.shape[0], attr)

        # Number of elements to select
        num_elements = 200

        # Generate random indices (ensure they are within the range of the array length)
        indices = np.random.choice(label_flat.size, num_elements, replace=False)

        # Select elements from each array using the same indices
        label_flat = label_flat[indices]
        pred_flat = pred_flat[indices]
        attr_array = attr_array[indices]

        preds_array.append(pred_flat) 
        gts_array.append(label_flat) 
        attrs_array.append(attr_array)
        
        
        avg_surface_distance = calculate_asd(prediction == i, label == i)
        
        avg_surface_distance_list.append(avg_surface_distance)
        class_thresholds = [5]

        surface_dice = compute_surface_dice(torch.tensor(mask_label), torch.tensor(mask_prediction), class_thresholds=class_thresholds)
        nsd_list.append(surface_dice.detach().numpy())
    ...
    return metric_list, cdr_dist, auc_score, nsd_list, avg_surface_distance_list,  \
        hausdorff_list, preds_array, gts_array, attrs_array
